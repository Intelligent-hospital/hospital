...
public interface DeptDoctorMapper {
	...
	@Results(id = "detorMap", value = { @Result(column = "id", property = "id"),
			@Result(column = "doname", property = "doname"), @Result(column = "dophone", property = "dophone"),
			@Result(column = "do_sex", property = "sex"),
			/* 根据 部门id 查询部门信息 */
			@Result(column = "de_id", property = "dept", javaType = Dept.class, one = @One(select = "findByDeptId")),
			/* 根据 职位ID 查询职位信息 */
			@Result(column = "stat_id", property = "station", javaType = Station.class, one = @One(select = "findByStationId")),
			/* 根据 账号ID 查询 账号信息 */
			@Result(column = "us_id", property = "uspass", javaType = Uspass.class, one = @One(select = "findByUspassId")),
			/* 根据工作时间ID 查询工作时间信息 */
			@Result(column = "w_id", property = "work", javaType = Work.class, one = @One(select = "findByWorkId")), })
